...
def main(args=None):
    ...
    try:
            while rclpy.ok():
                ...
                deg = int(input("Input angle to rotate(deg): ") )      #input :일단 화면에 출력, 타입:String
                
                current = float(degrees(node.pose.theta))
                goal = float(degrees(current + radians(deg)))
                print('current = "%f", goal="%f"' %(current, goal))
                
                tw.angular.z = radians(30)       #radians(15) : 초당 15도씩 돌아라
