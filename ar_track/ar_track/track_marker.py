...
from geometry_msgs.msg import Pose, Twist           #로봇을 움직여야 하니까
from ros2_aruco_interfaces.msg import ArucoMarkers          
from math import degrees, radians, sqrt, sin, cos, pi
from tf_transformations import euler_from_quaternion #, quaternion(회전 좌표계)
...
class TrackMarker(Node):
    ...
    def __init__(self):
        ...
        self.pose = Pose()      #Pose타입(6개-position.(x,y,z),orientation(x,y,z))
        self.tw   = Twist()
        self.tb3  = MoveTB3()
    ...
    def get_marker_pose_(self, msg):
        ...
        * p: euler_from_quaternion(q)[1] | pitch  (y) | (z) yaw ** | <-- 
          y: euler_from_quaternion(q)[2] | yaw    (z) | (x) roll   | 
                                         +------------+------------+
        """
        if len(msg.marker_ids) == 0:    # no marker found, 마커 못 찾음
            self.target_found = False
        
        else:               # if len(msg.marker_ids) != 0:  # marker found at least 1EA
        
            for i in range(len(msg.marker_ids)):            #마커 아이디의 개수만큼 반복
            
                if msg.marker_ids[i] == TARGET_ID:          # target marker found, 같은 마커가 있는지 검사
                    if self.target_found == False:          #타켓 아이디 검출 
                        self.target_found = True                        
                    self.pose  = msg.poses[i]               #지금 타켓아이디 마커의 정보
                    self.theta = self.get_theta(self.pose)  #self.theta:라디안값
                else:
                    self.target_found = False            
        
    def get_theta(self, msg):
        q = (msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.orientation.w)      #튜플 형태
        euler = euler_from_quaternion(q)        #리턴값3개(0,1,2)
        theta = euler[1]        #피치값(theta)
        return theta
